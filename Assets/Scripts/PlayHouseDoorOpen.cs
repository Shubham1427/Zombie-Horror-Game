...
public class PlayHouseDoorOpen : MonoBehaviour
{
    ...
    public Camera cam;
    private int active = 0, zomActive = 0, opened = 0;
    public int locked = 1;
    ...
    private void Update()
    {
        ...
        {
            if (hit.collider.gameObject == gameObject && locked == 0)
            {
        ...
        else if (zom.GetComponent<ZombieAI>().enabled)
        {
            ...
            {
                if (zomHit.collider.gameObject == gameObject && locked == 0)
                {
                    if (opened == 0)
                    {
                        hinge.GetComponent<Animation>().Play("DoorOpenAnim");
